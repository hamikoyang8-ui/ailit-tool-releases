...
1.商品上传页面滑动拖拽的时候有点卡.还有就是你改了页面里面的小滑动但是数据拖拽部分原来是锁定的,左侧拖拽栏做悬浮的跟随滚动行不行.2.图片下载为什么变成了webp,初始版本一个是用的原生接口 我下载的都是png,你修复反而变成了webp.我不喜欢这样越改越多bug。3,对比两个版本的调拨盘点模块,最新版有没有被改错了,盘点调拨不是像销售单一样没搜索到掉入原生,还是会弹出,现在增加一个盘点调拨模块可能更特殊一点是有无商品查到是可以提示新增商品的,做个三级 快速-原生(回车)点击候选就跳过,没找到原生自己该怎么跳原生的商品创建这个不管.
